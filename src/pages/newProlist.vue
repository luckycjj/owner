<template>
  <div id="newProlist" style="top:0rem;">
    ...
    <div id="showBox">
      <div id="titleBox">
        ...
        <div class="looktrackList" @touchend="looktrackList()">
           运单
        </div>
    ...
    <div id="gonewOrder" @touchend="gonewOrder()">
        发货
    </div>
...
<script>
  ...
  export default {
    ...
    data(){
      return{
        list:[{
          name:"发货",
          number:0,
          img:require("../images/prolistnewOrder.png"),
          prolist:[]
        },{
          name:"发货历史",
          number:0,
          img:require("../images/prolisthistroyOrder.png"),
          prolist:[]
        },{
          name:"常发货源",
          number:0,
          img:require("../images/prolistuserMore.png"),
          prolist:[]
        }],
        tabShow:0,
      }
    ...
    mounted:function () {
      ...
      androidIos.judgeIphoneX("titleBox",0);
    ...
    methods:{
      go:function () {
        var _this = this;
        _this.corner();
        var trackTap = sessionStorage.getItem("newProTap");
        ...
        $("#trackTab li").click(function(){
          ...
          sessionStorage.setItem("newProTap",i);
          if(curNavIndex!=i) {
            //更改列表条件
            //隐藏当前列表和回到顶部按钮
      ...
      lookTrackMore:function (pk,index) {
        var _this = this;
        androidIos.addPageList();
        _this.$router.push({ path: '/orderLogisticsMore3',query:{pk:pk,type:index == 0 ? 1 :3}});
      },
      corner:function () {
        var _this = this;
        $.ajax({
...
<style scoped>
  ...
  .wrapper .scroller li {
    ...
    font-size: .4rem;
    text-align: center;
    width:2.466666666667rem;
  }
  .wrapper .scroller li a{
    color:#999;
    display:block;
    font-size: 0.375rem;
    margin:0.98rem 0.1rem 0 0.1rem;
  }
  ...
  .looktrackList{
    ...
    background-image: url("../images/newPeolistlooktrack.png");
  }
  .looktrackList img{
    width:1rem;
    margin:0.2rem auto;
