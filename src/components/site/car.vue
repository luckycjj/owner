        ...
        if(!bomb.hasClass("downJian"+Zongtype,"logisticsImg")){
          ...
          function setListData(curPageData, num, size,typeNumber){
            ...
            for (var i = 0; i < curPageData.length; i++) {
              ...
              var img2 = _this.orderPk != "" ?"<div class='checkImg' style='display: "+display3+"'></div>":"";
              var str = "";
              str += '<div class="top" data-nowweight="'+pd.nowweight+'" data-nowCartype="'+pd.type+'" data-sWeight="'+androidIos.numSub(pd.zongweight ,pd.nowweight)+'" data-userNow="'+pd.userNow+'" data-driverLicense="'+pd.driverLicense+'" data-pkCar="'+pd.pkCar+'" data-carType="'+pd.carType+'">'+
                ...
                if($("#search").find("h5").text() != "取消"){
                  ...
                  if(_this.orderPk != ""){
                    ...
                    if(carModel.indexOf("整") != -1){
                      ...
                      var nowCartype = 0;
                      var nowCartypeData = that.attr("data-nowCartype");
                      ...
                      if(carPKlistGo.indexOf(pkcar) == -1){
                        ...
                        if(nowCartypeData == 1 || nowCartypeData == 2){
                          if(androidIos.numSub(nowweight,sessionStorage.getItem("weh"))> 0){
                            nowCartype = 1;
                          }
                        }
                      }
                      _this.$router.push({ path: '/car',query:{title: carModel,pkCar:pkcar,carType:cartype,nowCartype:nowCartype}});
                    }
                  }else{
                    if(that.parents("li").attr("data-nowtype") == '0'){
                      bomb.first( that.find(".carnumber").text() + "正在审核");
